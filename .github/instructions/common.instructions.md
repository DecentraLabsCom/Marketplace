...
3. **Hooks Architecture**: Custom hooks follow a modular, domain-based architecture that optimizes React Query's (for SSO) and Wagmi's (for wallet, built on React Query) caching and error handling capabilities:
   - **Index/Barrel Files**: Each domain **must** have a barrel file named `use[Domain].js` (e.g., `useLabs.js`, `useBookings.js`, `useStaking.js`) that re-exports all hooks for that domain. When adding a new hook domain, create the barrel file as part of the initial setup. Components should always import from these barrel files, never from individual hook files, to maintain architectural consistency and enable easier refactoring.
   - **Atomic Query Hooks**: Single `useQuery` calls that fetch specific data pieces. Three variants must be provided for each query: one for SSO users using API + Ethers (e.g., `useReservationSSO`) -with 1:1 relationship with API endpoints-, one for wallet users using Wagmi (e.g., `useReservationWallet`), and one router that auto-detects user type (e.g., `useReservation`). SSO variants must export a `.queryFn` for use in composed hooks. **IMPORTANT**: Wallet variants must normalize Wagmi data (BigInt, object structure) to match SSO format for consistency.
   - **Component Imports**: Components must always import and use the router variant (e.g., `useReservation`, not `useReservationSSO` or `useReservationWallet`). Direct SSO/Wallet variants are internal implementation details and should only be consumed by their own router hook or by composed hooks (via `.queryFn`).
   - **Atomic Mutation Hooks**: Single mutation operations. Three mutations must be provided for each operation: one for wallet-based actions (e.g., `useCancelBookingWallet`), one for SSO users (e.g., `useCancelBookingSSO`), and one to route to the appropriate implementation (Wallet or SSO; e.g., `useCancelBooking`). **Exception — wallet-only mutations**: When an operation inherently requires a connected wallet (e.g., staking tokens, provider admin actions), only a `Wallet` variant and a thin `Router` wrapper are needed. The router should be a direct pass-through with a comment explaining why SSO is not applicable.
   - **Composed and Specialized Query Hooks**: For complex data orchestration that requires multiple related queries, use `useQueries` to maintain React Query's caching, retry, and error handling benefits while providing unified data composition. These hooks use SSO variants exclusively (via `.queryFn`) because Wagmi hooks cannot be extracted as functions for `useQueries`. **CRITICAL**: Composed hooks must force `isSSO: true` mode because API endpoints are read-only blockchain queries that work for ANY wallet address (SSO or Wallet users). This allows both user types to benefit from composed hooks while maintaining the architectural constraint that `useQueries` cannot use Wagmi hooks. Atomic routers provide SSO/Wallet routing for simple cases where composition is not needed.
...
5. **Wagmi Integration**: The project uses Wagmi v.2 for Ethereum wallet connections and interactions from the client side. Ensure that all wallet-related functionality is implemented using Wagmi hooks and utilities.
   ...
   **Wallet Address Access**: Use `useConnection()` + `getConnectionAddress(connection)` from `@/utils/blockchain/connection` to obtain the connected wallet address. Do not use Wagmi's `useAccount()` directly — the project's connection utility provides a consistent abstraction layer over Wagmi's connection state.
...
14. **Shared Utilities and Constants**: Reusable logic that spans multiple components or hooks should be extracted to `src/utils/` (organized by subdomain, e.g., `blockchain/`, `formatting/`). Smart contract-derived display constants (e.g., thresholds, base values mirroring Solidity definitions) should be centralized in `src/constants/` with JSDoc referencing the originating contract and function. Avoid duplicating utility functions or hardcoding contract constants in components or hooks.