...
Use a reviewed global exception only for shared or external metadata infrastructure. Platform administrators can manage dynamic exceptions, including owner, reason and creation date, at `/api/admin/metadata-origin-exceptions`; revocation is immediate and does not require a deployment. As a bootstrap fallback, `ALLOWED_METADATA_ORIGINS` remains available: configure a comma-separated list of exact, credential-free HTTPS origins (scheme, host and port only; no path, query or fragment). These entries are combined with provider origins registered on-chain and reviewed dynamic exceptions; they must not be used as a substitute for an individual provider's registered backend origin. Local `Lab-*.json` metadata is read only from the repository `data/` directory and does not require `labId`.
