...
jest.mock('wagmi', () => ({
  useAccount: jest.fn(() => ({ address: '0x123', chain: { id: 1, name: 'ethereum' } })),
...
describe('useLabTokenHook', () => {
  ...
  beforeEach(() => {
    ...
    mockReadFactory.mockImplementation((fnName) => {
      if (fnName === 'balanceOf') return { data: 1000n, refetch: refetchBalanceSpy };
      if (fnName === 'allowance') return { data: 500n, refetch: refetchAllowanceSpy };
      if (fnName === 'decimals') return { data: 18 };
      return { data: undefined, refetch: jest.fn() };
    });

    // mock write function
    mockWriteFactory.mockImplementation(() => ({ contractWriteFunction: contractWriteSpy }));

    // default tx receipt state
    wagmi.useWaitForTransactionReceipt.mockReturnValue({ isLoading: false, isSuccess: false });
  });

  test('returns initial state correctly', () => {
    const { result } = renderHook(() => useLabToken(), { wrapper: createWrapper() });
  ...
  test('after receipt success it refetches balance & allowance', async () => {
    const { result, rerender } = renderHook(() => useLabToken(), { wrapper: createWrapper() });
    ...
    wagmi.useWaitForTransactionReceipt.mockReturnValue({ isLoading: false, isSuccess: true });
    rerender();
