...
describe("Carrousel Component", () => {
  // Test fixtures
  const mockLabWithImages = {
    images: ["/image1.jpg", "/image2.jpg", "/image3.jpg"],
  };
  const mockLabSingleImage = { images: ["/single.jpg"] };
  const mockLabEmpty = { images: [] };
  const mockLabWithNulls = {
    images: ["/image1.jpg", null, "/image2.jpg", undefined],
  };
  ...
  describe("Rendering", () => {
    test("renders placeholder image when no images available", () => {
      render(<Carrousel lab={mockLabEmpty} />);
      // Component now renders placeholder image instead of text
      const image = screen.getByTestId("media-display");
      expect(image).toBeInTheDocument();
      expect(image).toHaveAttribute("src", "/labs/lab_placeholder.png");
    });
    ...
    test("applies custom height when provided", () => {
      const { container } = render(
        <Carrousel lab={mockLabWithImages} maxHeight="500" />
      );
  ...
  describe("Edge cases", () => {
    test("handles null/undefined lab prop gracefully", () => {
      ...
      // Component now renders placeholder image instead of text
      let image = screen.getByTestId("media-display");
      expect(image).toBeInTheDocument();
      expect(image).toHaveAttribute("src", "/labs/lab_placeholder.png");

      rerender(<Carrousel lab={undefined} />);
      image = screen.getByTestId("media-display");
      expect(image).toBeInTheDocument();
      expect(image).toHaveAttribute("src", "/labs/lab_placeholder.png");
    });
