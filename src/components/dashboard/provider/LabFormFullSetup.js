...
/**
 * FMU-specific fields sub-section.
 * Provider enters fmuFileName and maxConcurrentUsers.
 * Auto-read fields (fmiVersion, simulationType, modelVariables, defaults) are fetched
 * from the gateway /describe endpoint and displayed as read-only.
 */
function FmuFieldsSection({ localLab, handleBasicChange, errors, disabled, gatewayUrl }) {
  ...
  return (
    <form className="space-y-6 text-gray-600" onSubmit={handleFormSubmit}>
      ...
      <div className="flex justify-between pt-4">
        ...
        >
          {localLab?.id ? 'Save Changes' : (localLab?.resourceType === RESOURCE_TYPES.FMU ? 'Add FMU Simulation' : 'Add Lab')}
        </button>
        <button
          type="button"
          onClick={onCancel}
