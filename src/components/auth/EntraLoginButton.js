...
export default function EntraLoginButton({ setIsModalOpen }) {
  ...
  const handleEntraLogin = (tenantId) => {
    if (typeof setIsModalOpen === 'function') {
      setIsModalOpen(false)
    }
    const url = tenantId
      ? `/api/auth/entra/login?tenant=${tenantId}`
      : '/api/auth/entra/login'
    router.push(url)
  }

  const institutionalTenantId = process.env.NEXT_PUBLIC_ENTRA_INSTITUTIONAL_TENANT_ID
  ...
  return (
    <div className="flex flex-col space-y-2">
      ...
      >
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-white/80">Sign in with any Microsoft account</p>
          </div>
      ...
      {institutionalTenantId && (
        <button
          onClick={() => handleEntraLogin(institutionalTenantId)}
          className="text-xs text-center text-neutral-500 hover:text-brand transition-colors py-1 underline-offset-2 hover:underline"
        >
          Institutional provider? Sign in with your organization account
        </button>
      )}
    </div>
  )
