...
export default function EntraLoginButton({ setIsModalOpen }) {
  ...
  const handleEntraLogin = (tenantId) => {
    ...
    const url = tenantId ? `/api/auth/entra/login?tenant=${tenantId}` : '/api/auth/entra/login'
    router.push(url)
  }
    ...
    <div className="flex flex-col space-y-2">
      ...
      >
        <div className="flex items-center space-x-4">
          ...
          {/* Chevron — mirrors InstitutionalLogin style */}
      ...
      {institutionalTenantId && (
        <button
          ...
          className="text-xs text-center text-neutral-500 hover:text-brand transition-colors py-1"
        >
          ¿Eres proveedor institucional? Entra aquí
        </button>
      )}
    </div>
  )
