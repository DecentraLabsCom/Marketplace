...
describe('/api/institutions/provisionConsumer compatibility route', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  test('creates a consumer pairing and ignores wallet/backend values from the browser', async () => {
    const { POST } = await import('../api/institutions/provisionConsumer/route.js');
    const response = await POST(new Request('https://marketplace.example/api/institutions/provisionConsumer', {
      method: 'POST',
      body: JSON.stringify({
    ...
    expect(mockCreatePairingForSession).toHaveBeenCalledWith('consumer', expect.anything());
