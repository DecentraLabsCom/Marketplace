/**
 * Tests for GET /api/contract/institution/getUserLimit
 *
...
function makeRequest(params = {}) {
  return { url: `http://localhost:3000/api/contract/institution/getUserLimit?${new URLSearchParams(params)}` }
}

describe('GET /api/contract/institution/getUserLimit', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    ...
    getContractInstance.mockResolvedValue({
      getInstitutionalUserLimit: jest.fn().mockResolvedValue(BigInt(5000)),
    })
  })
  ...
  it('returns 400 when institutionAddress is invalid', async () => {
    const res = await GET(makeRequest({ institutionAddress: 'bad' }))
    const data = await res.json()
    expect(res.status).toBe(400)
  })

  it('returns user limit on success', async () => {
  ...
  it('returns 500 on contract error', async () => {
    getContractInstance.mockResolvedValue({
      getInstitutionalUserLimit: jest.fn().mockRejectedValue(new Error('RPC Error')),
    })
    const res = await GET(makeRequest({ institutionAddress: VALID_ADDR }))
    const data = await res.json()
