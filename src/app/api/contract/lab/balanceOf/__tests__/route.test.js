/**
 * Tests for GET /api/contract/lab/balanceOf
 * Auth-gated, wallet param, viem isAddress validation, returns count.
 */
...
describe('GET /api/contract/lab/balanceOf', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('returns 400 when wallet address is invalid', async () => {
    const res = await GET(makeRequest({ wallet: 'not-an-address' }))
    const data = await res.json()
    expect(res.status).toBe(400)
  })

  it('returns lab count on success', async () => {
    const res = await GET(makeRequest({ wallet: VALID_ADDR }))
    const data = await res.json()
    expect(res.status).toBe(200)
    expect(data.count).toBe(3)
    expect(data.wallet).toBe(VALID_ADDR)
  })
