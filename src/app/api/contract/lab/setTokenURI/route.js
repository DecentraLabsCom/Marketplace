/**
 * API Route: /api/contract/lab/setTokenURI
 * Genera intent EIP-712 para actualizar el tokenURI de un lab y lo envía al gateway (si está configurado)
...
export async function POST(request) {
  ...
  try {
    ...
    const { labId, tokenURI, gatewayUrl: gatewayUrlOverride } = body;
    
    if (!labId && labId !== 0) {
      return Response.json({ error: 'Missing required field: labId', field: 'labId' }, { status: 400 });
    }
    if (!tokenURI) {
      return Response.json({ error: 'Missing required field: tokenURI', field: 'tokenURI' }, { status: 400 });
    }
    ...
    devLog.log('?? Building lab setTokenURI intent via SSO:', { labId: numericLabId });
    ...
    const processingTime = Date.now() - startTime;
    devLog.error('? Error building setTokenURI intent via SSO:', error);
    ...
    return Response.json({
      ...
      message: process.env.NODE_ENV === 'development' ? error.message : 'Unexpected error while creating setTokenURI intent',
      processingTime
    }, { status: 500 });
  }
}
