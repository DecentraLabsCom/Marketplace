/**
 * API Route: /api/contract/lab/listLab
 * Genera intent EIP-712 para listar un lab y lo envía al gateway (si está configurado)
...
export async function POST(request) {
  ...
  try {
    ...
    devLog.log('?? Building lab list intent via SSO:', { labId: numericLabId });
    ...
    const processingTime = Date.now() - startTime;
    devLog.error('? Error building lab list intent via SSO:', error);
    ...
    return Response.json({
      ...
      message: process.env.NODE_ENV === 'development' ? error.message : 'Unexpected error while creating list intent',
      processingTime
    }, { status: 500 });
  }
}
