...
describe('institutionToasts', () => {
  ...
  test('emits provisioning-related toasts with unified notification signature', () => {
    ...
    expect(calls[0][1]).toContain('pairing challenge')
    expect(calls[1][1]).toContain('pairing challenge')
    ...
    expect(calls[3][1]).toContain('Pairing challenge created')
    expect(calls[4][1]).toContain('Pairing challenge created')
    expect(calls[3][1]).not.toContain('token')
    expect(calls[4][1]).not.toContain('token')
