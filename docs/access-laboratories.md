...
The Marketplace session uses a sliding 60-minute inactivity window. Authenticated Marketplace requests refresh that window, including its SAML single-logout binding, when it is close to expiring; a periodic status check by itself does not keep an otherwise idle session alive.
